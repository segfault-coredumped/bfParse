...
This project is a simple implementation of a recursive descent parser for BF-like programs. The parser interprets the input BF code, builds a parse tree, and then interprets the program according to the BF instructions.
...
- **Parser**: A recursive descent parser that builds a parse tree for BF-like syntax.
- **Interpreter**: A simple interpreter that executes the parse tree and outputs the result.
- **Output**: A function to display the parse tree and the interpreted BF program.

## Features

- **Parse Tree**: The parser generates a recursive descent parse tree for valid BF-like programs.
- **Interpreter**: The interpreter executes the parsed program, manipulating memory and output as specified by the BF language.
- **Program Output**: The program outputs both the original BF code, the parse tree, and the result of running the program.
...
These functions implement the recursive descent parsing strategy for the BF-like grammar:
...
The `interpret` function recursively interprets the parsed tree. It handles all BF commands such as `+`, `-`, `>`, `<`, `.`, and `,`. It uses a memory array (`data`) to simulate the tape of a BF interpreter.
...
- Reads the BF code from `bf2.txt`.
- Removes any non-BF characters (e.g., comments or whitespace).
